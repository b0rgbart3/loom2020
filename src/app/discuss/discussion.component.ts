...
export class DiscussionComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.ds.threadAdded.subscribe(thread => {
      ...
      if ((this.thisClass.id === thread.classId) && (+thread.section === +this.section.sectionNumber)
      ) {
  ...
  checkSequenceAndFolds(): void {
    ...
    for (let i = 0; i < this.threads.length - 1; i++) {
      const timeStampA = this.threads[i].postDate;
      const timeStampB = this.threads[i + 1].postDate;
  ...
  newThread(): void {
    ...
    if (this.discussionFormGroup.valid && this.discussionFormGroup.dirty) {
      ...
      combinedObject.userId = this.currentUser.id;
      combinedObject.postDate = new Date(Date.now());
  ...
  foldChange(thisThread: Thread): void {
  ...
  threadChange(thisThread: Thread): void {
  ...
  deleteThread(thisThread: Thread): void {
  ...
  removeThread(thisThread: Thread): void {
    let foundIndex = 0;
